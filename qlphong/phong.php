...
if ($selected_toanha_id > 0) {
    $stmt = $conn->prepare("SELECT toanha FROM toanha WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // DEBUG CHI TIẾT
    error_log("DEBUG FORM - edit_id: " . $edit_id . ", id_toanha: " . $id_toanha . ", tenphong: " . $tenphong);
    error_log("DEBUG POST DATA: " . print_r($_POST, true));
    
    if ($edit_id > 0) {
        // CHỈNH SỬA PHÒNG
        $stmt = $conn->prepare("UPDATE phong SET tinhtrang = ?, trangthaihoatdong = ? WHERE id = ?");
        $stmt->bind_param("ssi", $tinhtrang, $trangthaihoatdong, $edit_id);
        
        if ($stmt->execute()) {
            $toast_message = "Cập nhật phòng thành công!";
            $toast_type = 'success';
            echo "<script>setTimeout(() => window.location.href = 'phong.php" . ($selected_toanha_id > 0 ? "?toanha_id=" . $selected_toanha_id : "") . "', 1000);</script>";
        } else {
            $toast_message = "Lỗi khi cập nhật: " . $conn->error;
            $toast_type = 'error';
        }
        $stmt->close();
    } else {
        // THÊM MỚI PHÒNG - KIỂM TRA KỸ HƠN
        error_log("DEBUG ADD MODE - id_toanha: " . $id_toanha . ", tenphong: '" . $tenphong . "'");
        
        // Nếu không có id_toanha từ form nhưng có selected_toanha_id, sử dụng cái đó
        if ($id_toanha <= 0 && $selected_toanha_id > 0) {
            $id_toanha = $selected_toanha_id;
            error_log("DEBUG - Using selected_toanha_id: " . $id_toanha);
        }
        
        if (empty($tenphong)) {
            $toast_message = "Vui lòng nhập tên phòng!";
            $toast_type = 'error';
            error_log("ERROR - tenphong is empty");
        } elseif ($id_toanha <= 0) {
            $toast_message = "Vui lòng chọn tòa nhà!";
            $toast_type = 'error';
            error_log("ERROR - id_toanha is invalid: " . $id_toanha);
        } else {
            // KIỂM TRA TRÙNG TÊN
            $check_stmt = $conn->prepare("SELECT id FROM phong WHERE idtoanha = ? AND tenphong = ?");
            $check_stmt->bind_param("is", $id_toanha, $tenphong);
            $check_stmt->execute();
            $check_result = $check_stmt->get_result();
            
            if ($check_result->num_rows > 0) {
                $toast_message = "Tên phòng '$tenphong' đã tồn tại trong tòa nhà này!";
                $toast_type = 'error';
            } else {
                $stmt = $conn->prepare("INSERT INTO phong (idtoanha, tenphong, tinhtrang, trangthaihoatdong) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("isss", $id_toanha, $tenphong, $tinhtrang, $trangthaihoatdong);
                
                if ($stmt->execute()) {
                    $toast_message = "Thêm phòng thành công!";
                    $toast_type = 'success';
                    echo "<script>setTimeout(() => window.location.href = 'phong.php" . ($selected_toanha_id > 0 ? "?toanha_id=" . $selected_toanha_id : "") . "', 1000);</script>";
                } else {
                    $toast_message = "Lỗi khi thêm phòng: " . $conn->error;
                    $toast_type = 'error';
                }
                $stmt->close();
            }
...
if (isset($_GET['delete_id'])) {
    ...
    if ($stmt->execute()) {
        $toast_message = "Xóa phòng thành công!";
        $toast_type = 'success';
        echo "<script>setTimeout(() => window.location.href = 'phong.php" . ($selected_toanha_id > 0 ? "?toanha_id=" . $selected_toanha_id : "") . "', 1000);</script>";
    } else {
        $toast_message = "Lỗi khi xóa: " . $conn->error;
        $toast_type = 'error';
    }
    $stmt->close();
}
...
$query = "SELECT p.*, t.toanha as ten_toa_nha FROM phong p JOIN toanha t ON p.idtoanha = t.id WHERE 1=1";
$params = []; $types = "";
...
if (!empty($search_keyword)) {
    $query .= " AND (p.tenphong LIKE ? OR t.toanha LIKE ?)";
    $search_param = "%$search_keyword%";
    $params[] = $search_param; $params[] = $search_param; $types .= "ss";
}

$query .= " ORDER BY t.toanha, p.tenphong ASC";
...
$toa_nha_result = $conn->query("SELECT id, toanha FROM toanha ORDER BY toanha");
?>
                    ...
                    <div class="filter-group">
                        ...
                        <div class="search-container">
                            <input type="text" id="search" name="search" value="<?php echo htmlspecialchars($search_keyword); ?>" 
                                   placeholder="Tìm theo tên phòng..." oninput="handleSearchInput()">
                ...
                <tbody>
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php $stt = 1; while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                <td><?php echo $stt++; ?></td>
                                <td><?php echo htmlspecialchars($row['ten_toa_nha']); ?></td>
                                ...
                                <td>
                                    <a href="svphong.php?phong_id=<?php echo $row['id']; ?>">
                                    ...
                                    <button class="action-btn delete" onclick="deleteRoom(<?php echo $row['id']; ?>)">Xóa</button>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
            ...
            <form method="POST" id="roomForm">
                ...
                <div class="form-group">
                    ...
                    <select id="idtoanha" name="idtoanha" required <?php echo ($selected_toanha_id > 0) ? 'disabled' : ''; ?>>
                        ...
                        ?>
                            <option value="<?php echo $toa_nha['id']; ?>" 
                                <?php echo ($selected_toanha_id > 0 && $toa_nha['id'] == $selected_toanha_id) ? 'selected' : ''; ?>>
