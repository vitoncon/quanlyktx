...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_building'])) {
    ...
    if (!empty($ten_toa_nha)) {
        ...
        $check_stmt = $conn->prepare("SELECT id FROM toanha WHERE toanha = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_building'])) {
    ...
    if ($id > 0) {
        $stmt = $conn->prepare("UPDATE toanha SET tinhtrang = ?, trangthaihoatdong = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM toanha WHERE id = ?");
                    ...
                    <div class="filter-group">
                        ...
                        <div class="search-container">
                            <input type="text" id="search" name="search" value="<?php echo htmlspecialchars($search_keyword); ?>" 
                                   placeholder="Tìm theo tên tòa nhà, tình trạng..." 
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php 
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="phong.php?toanha_id=<?php echo $row['id']; ?>">
                                        <button class="action-btn view">Xem</button>
                                    </a>
                                    <button class="action-btn edit" onclick="openBuildingPopup(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars($row['toanha']); ?>', '<?php echo $row['tinhtrang']; ?>', '<?php echo $row['trangthaihoatdong']; ?>')">Sửa</button>
                                    <button class="action-btn delete" onclick="deleteBuilding(<?php echo $row['id']; ?>)">Xóa</button>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
